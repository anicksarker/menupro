...
calc_engine_34.py
===================
Self-contained Plan 34 workbook builder -- same shape as calc_engine_v9.py:
one file, reads fixed CSV filenames, runs at import time, writes
WL_model_34.xlsx. Drop it in next to main_v2.py, point CALC_ENGINE_FILE
at it, nothing else changes.

CONTRACT (matches what main_v2.py's run_calc_engine() expects):
  - No CLI arguments. Reads fixed filenames from its own directory.
  - Reads:  Inputs_v2.csv  (label,value pairs -- whatever
            main_v2.build_inputs_csv() wrote for the current policy)
            cso_rates.csv  (static mortality-rate table, same file
            every run -- not regenerated per policy)
  - Writes: WL_model_34.xlsx  (picked up by main_v2.get_latest_model_file()
            via its WL_model*.xlsx glob)
  - Runs its logic at import time (main_v2.run_calc_engine() calls
    spec.loader.exec_module(module), which executes this file top to
    bottom -- there's no __main__ guard, same as calc_engine_v9.py,
    because exec_module() doesn't set __name__ to "__main__" so that
    guard would silently never fire under this calling convention).

WHY THIS IS DIFFERENT FROM calc_engine_v9.py
------------------------------------------------
calc_engine_v9.py hand-reconstructs every formula from scratch (see its
own comments: "Key fixes: 1) correct operator symbols 2) correct
scaling"), and invents inputs that don't exist in the real module 34
reference workbook (e.g. "Expense Loading Factor", "Paid-Up Age" --
module 34 is a PPT/limited-pay product, there is no paid-up-age
concept in it). It may produce plausible-looking numbers, but nothing
in it was checked against a real reference workbook.

This script instead embeds data extracted directly from your actual
Plan 34 reference workbook, cell-by-cell, and verified formula-for-
formula (0 mismatches across 47,817 checked cells against the source
workbook). Concretely, every formula in every calculation sheet
(MORTALITY, CV_RATES, PUA_TABLE, Projection (Annual), Projection_Monthly,
RPU, ETI, APL, Commutation, Loan, DIvidend_Assumption, TERM_RATES) was:

  1. Tokenized into (text / cell-reference / bare-number) parts.
  2. Grouped by structural "shape" per column (same literal text, same
     referenced columns/sheets, same $-anchoring), ignoring row numbers.
  3. Within each shape-group, checked for a constant row-to-row offset
     in any relative reference or bare integer, replaced with a
     placeholder like {R+3} -- e.g. template "=+A{R-1}+1" at row 5
     renders to "=+A4+1". This is exactly how Excel's own fill-down
     works (R1C1-relative addressing); the FUNCTIONAL_SHEETS data below
     just writes that relationship out explicitly instead of leaving it
     implicit in a spreadsheet UI.
  4. Verified: every reconstructed formula matches the original formula
     text *exactly*, for every row it covers.
  5. The ~437 formulas that didn't fit a clean row-linear pattern
     (mostly one-off header/edge rows) are stored verbatim per cell.

Net: ~30,000 formula cells -> ~211 reusable templates + 437 explicit
formulas + ~618 static label cells, all plain, readable Python below
(search for any cell address, e.g. "A2", to see exactly what it gets).

TWO THINGS ADDED ON TOP OF THE REFERENCE WORKBOOK
------------------------------------------------------
1. Inputs_v2.csv is a 2-column label/value CSV (main_v2.py's format),
   not the wide CSV layout the reference workbook's own Inputs sheet
   export uses -- so Inputs are populated by matching LABEL TEXT
   (LABEL_TO_ROW below), not column position.
2. 'Loan Outstanding Advance' / 'Loan Outstanding Arrear' rows (Inputs
   D28:E29) -- not present in the original reference workbook, added
   because main_v2.SUMMARY_LABELS expects them. They pull the last
   non-blank "Outstanding After" balance from the real Loan sheet's
   Advance-method column (G28:G40) and Arrear-method column (P28:P40)
   -- confirmed against the actual Loan sheet layout, not guessed.

IMPORTANT CAVEAT ON THE LOAN SHEET
-------------------------------------
The reference workbook's Loan sheet is NOT formula-driven off Inputs/
CSO Rates the way every other sheet is -- its loan transactions (dates,
amounts) are manually entered example data for one specific policy.
Neither Inputs_v2.csv nor this script feeds real loan transactions in
per policy. If you need genuine per-policy loan history reflected in
"Loan Outstanding Advance/Arrear", that's a separate data-wiring task
(e.g. pulling from a loan-ledger extract per policy) that hasn't been
built yet -- every policy run currently carries the same example loan
data baked into the Loan sheet below.

REQUIRES: openpyxl only (no other local files -- this is the whole
engine in one script, same as calc_engine_v9.py).
"""
...
# ========================================================================
# ENGINE: reads Inputs_v2.csv + cso_rates.csv, builds the workbook
# ========================================================================
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
INPUT_CSV = os.path.join(BASE_DIR, "Inputs_v2.csv")
CSO_CSV = os.path.join(BASE_DIR, "cso_rates.csv")
OUTPUT_XLSX = os.path.join(BASE_DIR, "WL_model_34.xlsx")

# ----------------------------------------------------------------------
# Reference workbook's 'Inputs' sheet layout: label (col A) -> row.
# Only the rows that are RAW INPUTS are listed -- rows that are
# themselves live formulas in the template (Issue Age, Current age,
# Duration(x), Duration(x+1), Base SA, Modal Premium, Modal Factor,
# Premium Paid-Up Duration, Is Currently Paying Premium) are handled
# automatically via INPUTS_FORMULAS and must NOT appear here.
# ----------------------------------------------------------------------
LABEL_TO_ROW = {
    "Mortality Table": 2,
...
def build_inputs_sheet_from_labels(ws, label_values: dict):
    ...
    # (FFB3FBB6, the "Output" block). Everything else on this sheet
    # outside the raw input rows (dividend lookup table, matched
    # policies table, legend/tag columns) is neither a CSV input nor
    # part of the green-marked output block, so it's not written here.
    #
...
def main():
    if not os.path.exists(INPUT_CSV):
        raise FileNotFoundError(f"Inputs template not found: {INPUT_CSV}")
    if not os.path.exists(CSO_CSV):
        raise FileNotFoundError(f"CSO rates file not found: {CSO_CSV}")

    label_values = load_label_value_csv(INPUT_CSV)
    ...
    build_cso_sheet(wb["CSO Rates"], load_csv_grid(CSO_CSV))
...
# Runs at import/exec time, matching calc_engine_v9.py's convention
# (main_v2.run_calc_engine() uses spec.loader.exec_module()).
main()
